...
interface CallDao {
    ...
    @Query("SELECT * FROM calls WHERE user_id = :userId ORDER BY created_millis DESC")
    suspend fun getByUserId(userId: Long): List<CallEntity>

    @Query("SELECT * FROM calls ORDER BY created_millis DESC")
    ...
    @Query("""
        SELECT 
            strftime('%Y-%m-%d', cm.start_time_millis/1000, 'unixepoch', 'localtime') AS period,
        ...
        WHERE cm.start_time_millis BETWEEN :startMillis AND :endMillis
        GROUP BY period
    ...
    @Query("""
        SELECT 
            strftime('%Y-%W', cm.start_time_millis/1000, 'unixepoch', 'localtime') AS period,
            COUNT(DISTINCT c.id) as total,
            SUM(CASE WHEN c.status = 'COMPLETED' THEN 1 ELSE 0 END) as answered,
            SUM(CASE WHEN c.status = 'DROPPED' THEN 1 ELSE 0 END) as missed,
            SUM(CASE WHEN dr.probability >= :threshold THEN 1 ELSE 0 END) as suspicious,
